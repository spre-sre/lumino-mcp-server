...
# Initialize Kubernetes API clients only if config was loaded successfully
if k8s_config_loaded:
    ...
    # Only attempt optional API clients if the core clients were initialised
    if k8s_core_api is not None:
        # AutoscalingV2Api may be unavailable if cluster lacks the autoscaling/v2 API group
        try:
            k8s_autoscaling_api = client.AutoscalingV2Api()
        except Exception as e:
            logger.warning(f"Failed to initialize AutoscalingV2Api: {e}")
            k8s_autoscaling_api = None

        # NetworkingV1Api for Ingress support (KubeArchive discovery on plain Kubernetes)
        try:
...
# Guard checks the two required params; k8s_networking_api is Optional in the
# constructor (None disables Ingress-based lookup) so it is not checked here.
...
def _require_k8s_client(client: object, name: str = "k8s_core_api") -> None:
    ...
    Call at the top of any tool function that needs a specific k8s client,
    e.g. ``_require_k8s_client(k8s_core_api, "k8s_core_api")``.
    """
    if client is None:
        raise RuntimeError(
            f"Kubernetes API client '{name}' is not initialized. "
            f"Ensure a valid kubeconfig or in-cluster configuration is available."
        )
...
async def list_namespaces() -> List[str]:
    ...
    if not k8s_core_api:
        logger.warning(
            ...
            "Ensure a valid kubeconfig or in-cluster configuration is present."
        )
        return []
...
async def list_pipelineruns(
    ...
    """
    _require_k8s_client(k8s_custom_api, "k8s_custom_api")
    try:
...
def get_kubernetes_resource(
    ...
    """
    _require_k8s_client(k8s_core_api, "k8s_core_api")
    try:
        ...
        if resource_type in core_resources:
            ...
            if not k8s_autoscaling_api:
                raise RuntimeError(
                    "AutoscalingV2Api is not available on this cluster. "
                    "The autoscaling/v2 API group may not be supported."
                )
...
async def _discover_prometheus_endpoint(
    ...
    for cluster_name, prom_config in OPENSHIFT_PROMETHEUS_ENDPOINTS.items():
        if cluster_name != "local":
            endpoint = prom_config.get("url")
            if endpoint:
                endpoint_type = prom_config.get("type", "prometheus")
